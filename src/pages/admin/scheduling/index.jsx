...
import { Button, DatePicker, Pagination, Select, SelectItem, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow } from '@heroui/react'
                    ...
                    <TableBody>
                        {classes.map((classItem) => (
                            <TableRow key={classItem.id} >
                                ...
                                <TableCell>
                                    <p className={`p-2 w-full text-center rounded-md ${classItem.status === "Completed" || classItem.status === "Upcoming" ? `text-[${classItem.complete}] bg-[${classItem.completebg}]` : `text-[${classItem.cencelled}] bg-[${classItem.cancelledbg}]`} `}>{classItem.status}</p>
                                </TableCell>
                                <TableCell>
                                   <div className="flex gap-1">
                                     <span><Copy color="#3F86F2" size={20} /></span>
                                    <span className="text-[#3F86F2]">Copy link</span>
                                   </div>
                                </TableCell>
                                <TableCell className="flex items-center gap-2">
                                    {/* <CourseForm initialData={classItem} /> */}
                                    <Button radius="sm" variant="bordered" className="border-[#06574C] " startContent={<Calendar size={20} color="#06574C" />}>
                                        Reschedule
                                    </Button>
