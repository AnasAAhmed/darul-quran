...
import { BookIcon, CalendarIcon, ChartBarIcon, DollarSignIcon, FileQuestionIcon, HomeIcon, MegaphoneIcon, TicketIcon, UsersIcon } from 'lucide-react';
...
const Sidebar = ({ isSidebarOpen,setIsSidebarOpen }) => {
  ...
  return (
    <div className="w-72 h-screen bg-[#1a5850]  text-white flex flex-col">
      <div className="p-6 mx-auto border-b border-white/10">
        <img src="/icons/logo.png" onClick={()=>  setIsSidebarOpen(!isSidebarOpen)} alt="Darul Quran" className=' w-36 h-36' />
      </div>
          ...
          {menuItems.map((item, idx) => (
            <li key={idx}>
              ...
              >
                ...
                {item.badge && (
                  <span className="px-2 py-0.5 text-xs bg-[#EBD4C90] text-[#06574C] rounded-full min-w-5 text-center">
                    {item.badge}
                  </span>
                )}
                ...
                {/* {item.children && (
                  <motion.svg
                    animate={{ rotate: expandedItems.includes(idx) ? 180 : 0 }}
                    ...
                    {item.children.map((child, childIdx) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="absolute rounded-xs -top-4 left-9 w-3 h-9 border-l-2 border-b-2 border-[#ffff] /30"></span>
                          {child.name}
                        </div>
                      </li>
                    ))}
